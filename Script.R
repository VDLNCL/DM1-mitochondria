#New scrip for permutation test
#Prepare data
...
#Rename samples 
samples= c("CF NPC", "CF OXPHOS", "CG NPC", "CG OXPHOS", "CH NPC", "CH OXPHOS", 
           "P1242_PRE NPC", "P1242_PRE OXPHOS", "P1242_POST NPC", "P1242_POST OXPHOS",
           "P1806_PRE NPC", "P1806_PRE OXPHOS", "P1806_POST NPC", "P1806_POST OXPHOS", 
           "P0907_PRE NPC", "P0907_PRE OXPHOS", "P0907_POST NPC", "P0907_POST OXPHOS", 
           "P1791_PRE NPC", "P1791_PRE OXPHOS", "P1791_POST NPC", "P1791_POST OXPHOS", 
           "P2182_PRE NPC", "P2182_PRE OXPHOS", "P2182_POST NPC", "P2182_POST OXPHOS", 
           "P0523_PRE NPC", "P0523_PRE OXPHOS", "P0523_POST NPC", "P0523_POST OXPHOS",
           "P1955_PRE NPC", "P1955_PRE OXPHOS", "P1955_POST NPC", "P1955_POST OXPHOS", 
           "P2002_POST NPC", "P2002_POST OXPHOS", 
           "P2005_PRE NPC", "P2005_PRE OXPHOS", "P2005_POST NPC", "P2005_POST OXPHOS", 
           "P2019_PRE NPC", "P2019_PRE OXPHOS", "P2019_POST NPC", "P2019_POST OXPHOS",
           "P2110_PRE NPC", "P2110_PRE OXPHOS", "P2110_POST NPC", "P2110_POST OXPHOS")
...
dat$PatientID = substr(dat$SectionID,1,5)
...
for(id in sort(unique(dat$SectionID))){
  ...
  if(dim(dt)[1] > 0){
    ...
    for(ch in c("NDUFB8","MTCO1")){
      ...
      controls = do.call("rbind",wides[c("CF","CG","CH")])
...
summ = data.frame(ID=ID,CHAN=CHAN,UP=UP,DOWN=DOWN,stringsAsFactors=FALSE)
levls = c("CF","CG","CH")
for(i in 1:11){
  levls = c(levls,sprintf("P%04d_PRE",i))
  levls = c(levls,list.dirs("P%04d_POST",i))
}

png("Summary_Script.png",width=1200,height=1200,pointsize=16)
op = par(mfrow=c(2,2),mai=c(1.4,1.1,0.5,0.05))
for(ch in c("NDUFB8","MTCO1")){
  ch_summ = summ[summ$CHAN==ch,]
  ch_summ$ID = factor(ch_summ$ID,level=levls)
  stripchart(ch_summ$UP~ch_summ$ID,vertical=TRUE,las=2,ylab="Proportion above controls",main=ch,cex.axis=1.2,cex.lab=1.5,pch=16,cex=2,col=rgb(0.45,0,0.3),ylim=c(0,0.6))
  abline(v=c(seq(3.5,25,2)),lty=2)
  stripchart(ch_summ$DOWN~ch_summ$ID,vertical=TRUE,las=2,ylab="Proportion below controls",main=ch,cex.axis=1.2,cex.lab=1.5,pch=16,cex=2,col=rgb(0.45,0,0.3),ylim=c(0,0.6))
  abline(v=c(seq(3.5,25,2)),lty=2)
}
par(op)
dev.off()

c(rgb(0.45,0,0.3),rgb(0,0.5,0.3))
...
calcDiff = function(comb,ctdf,atype="normal",makeplot=FALSE,mitochan="VDAC1",id=""){
  ...
  for(trt in c("PRE","POST")){
    ...
    for(ch in pchans){
      ...
      pred = predict(mod, newdata=data.frame(xctrl=log(dt[[mitochan]])), se.fit=TRUE,  
                     interval = "prediction",na.action=na.omit)$fit
      up = pred[,3]
      low = pred[,2]	 
      dt[[paste(ch,"up_pred",sep="_")]]=up
      dt[[paste(ch,"low_pred",sep="_")]]=low
      ...
      if(makeplot){
        ...
        pred_syn = predict(mod,newdata = data.frame(xctrl=xsyn), se.fit=TRUE,  
                           interval = "prediction",na.action=na.omit)$fit
        mid_syn = pred_syn[,1]
        up_syn = pred_syn[,3]
        low_syn = pred_syn[,2]
...
pats=c("P1242", "P1806", "P0907", "P1791", "P2182", "P0523", "P1955", "P2019", "P2005", "P2110")
...
for(chan in c("NDUFB8","MTCO1")){
  for(direction in c("UP","DOWN")){
    ...
    stripchart(pres,vertical=TRUE,pch=16,col=rgb(0.00,0.19,0.56,0.075),method="jitter",jitter=0.2,las=2,ylab=ylab,main=chan,cex.axis=1.5,cex.lab=1.5,cex=0.5,ylim=c(-0.6,0.15))
    abline(h=0.0,lwd=2,lty=2)
    #text(seq(1,length(pats)),rep(0.05*(par("usr")[4]-par("usr")[3])+par("usr")[3],length(pats)),txt,cex=2,col="blue" )
    text(seq(1,length(pats)),rep(0.05*(par("usr")[4]-par("usr")[3])+par("usr")[3],length(pats)),qtxt,cex=3,col="black")
...
par(op)
dev.off()
write.csv(pvals, "p Values.csv")
write.csv(qvals, "q Values.csv")
